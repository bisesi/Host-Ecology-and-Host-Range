...
alphas_coop <- expand.grid(alpha2 = s_alpha2, alpha1 = e_alpha1)%>%
...
mus_coop <- expand.grid(rate_s = rate_s, rate_e = rate_e) %>%
...
gamma_and_rate_coop <- expand.grid(rate_s = rate_s, gamma_sp = gamma_sp) %>%
...
c_and_rate_coop <- expand.grid(rate_s = rate_s, c_sp = c_sp) %>%
...
r_and_gamma_coop <- expand.grid(gamma_sp = gamma_sp, R = R) %>%
  group_by(gamma_sp, R) %>%
  do(
    {
      ode(func=generalLV_coop_R,y=start_density,times=time,
          parms=c(alpha1 = 1, alpha2 = 1, rate_e = 0.5, rate_s = 0.5,
                  gamma_genE = 20, gamma_genS = 20,
                  gamma_sp = .$gamma_sp, c_genE = 1e-3, c_genS = 1e-3, c_sp = 1e-3,
                  dilution_E = 3e-2, dilution_S = 3e-2, dilution_gen = 3e-2, dilution_sp = 3e-2, k_e = 1, k_s = 1,R = .$R)) %>%
        as.data.frame()
    }
  )
...
r_and_c_coop <- expand.grid(c_sp = c_sp, R = R) %>%
  group_by(c_sp, R) %>%
  do(
    {
      ode(func=generalLV_coop_R,y=start_density,times=time,
...
#dilutiuon rate only
dilution_gen = seq(from = 0, to = parameters_coop_R['dilution_sp'] * maxcost, by = parameters_coop_R['dilution_sp'] / 20)
  ...
  do(
    {
      ode(func=generalLV_coop_R,y=start_density,times=time,
          parms=c(alpha1 = 1, alpha2 = 1, rate_e = 0.5, rate_s = 0.5,
                  gamma_genE = 20, gamma_genS = 20,
                  gamma_sp = 20, c_genE = 1e-3, c_genS = 1e-3, c_sp = 1e-3,
                  dilution_E = 1e-2, dilution_S = 1e-2, dilution_gen = .$dilution_gen, dilution_sp = 1e-2,
                  k_e = 1, k_s = 1, R = 1)) %>%
        as.data.frame()
    }
  )

#dilutiuon and gamma rate only
dilution_gen = seq(from = 0, to = parameters_coop_R['dilution_sp'] * maxcost, by = parameters_coop_R['dilution_sp'] / 10)
gamma_sp = seq(from = 0, to = parameters_coop_R['gamma_sp'] * maxcost, by = parameters_coop_R['gamma_sp'] / 10)

dilution_gamma_coop <- expand.grid(dilution_gen = dilution_gen, gamma_sp = gamma_sp)%>%
...
#dilutiuon and c rate only
dilution_gen = seq(from = 0, to = parameters_coop_R['dilution_sp'] * maxcost, by = parameters_coop_R['dilution_sp'] / 10)
c_sp = seq(from = 0, to = parameters_coop_R['c_sp'] * maxcost, by = parameters_coop_R['c_sp'] / 10)

dilution_c_coop <- expand.grid(dilution_gen = dilution_gen, c_sp = c_sp)%>%
