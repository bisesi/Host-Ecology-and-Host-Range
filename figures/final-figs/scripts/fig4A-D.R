...
partA <- pfus_and_final_density %>%
  ...
  rbind(., all_tecan_adjusted_OD %>% filter(interaction == "Fac" | interaction == "Smono") %>% 
...
partB <- pfus_and_final_density %>%
  ...
  mutate(interaction = case_when(interaction == "Fac" ~ "Facilitation",
                                                                interaction == "Smono" ~ "S Monoculture",
                                                                TRUE ~ interaction)) %>%
  filter(interaction == "Facilitation" | interaction == "S Monoculture") %>%
...
partD <- pfus_and_final_density %>%
  mutate(doublings = case_when(doublings == 0.0 ~ Inf,
                               TRUE ~ doublings)) %>%
  filter(interaction == "S Monoculture" | interaction == "Facilitation")%>%
...
#fig 4
fig4 <- (partA + partB) / (partC + partD) + plot_layout(guides = "collect") + plot_annotation(tag_levels = "A")
