#ATB
#coexistence graphs - cooperation
...
burst_size <- cycled_gamma_coop %>% mutate(coexistence = case_when(final_gen < 0.1 & final_sp < 0.1 ~ "Extinction",
...
consumption <- cycled_c_coop %>% mutate(coexistence = case_when(final_gen < 0.1 & final_sp < 0.1 ~ "Extinction",
                                                                final_gen > 0.1 & final_sp < 0.1 ~ "Generalist Only",
                                                                final_gen < 0.1 & final_sp > 0.1 ~ "Specialist Only",
                                                                final_gen > 0.1 & final_sp > 0.1 ~ "Coexistence"))%>%
...
mus <- cycled_mus %>% mutate(coexistence = case_when(final_gen < 0.1 & final_sp < 0.1 ~ "Extinction",
                                                     final_gen > 0.1 & final_sp < 0.1 ~ "Generalist Only",
                                                     final_gen < 0.1 & final_sp > 0.1 ~ "Specialist Only",
                                                     final_gen > 0.1 & final_sp > 0.1 ~ "Coexistence")) %>%
...
alpha <- cycled_alpha %>% mutate(coexistence = case_when(final_gen < 0.1 & final_sp < 0.1 ~ "Extinction",
                                                         final_gen > 0.1 & final_sp < 0.1 ~ "Generalist Only",
                                                         final_gen < 0.1 & final_sp > 0.1 ~ "Specialist Only",
                                                         final_gen > 0.1 & final_sp > 0.1 ~ "Coexistence")) %>%
...
mus_gamma <- mu_gamma %>% mutate(coexistence = case_when(final_gen < 0.1 & final_sp < 0.1 ~ "Extinction",
                                                         final_gen > 0.1 & final_sp < 0.1 ~ "Generalist Only",
                                                         final_gen < 0.1 & final_sp > 0.1 ~ "Specialist Only",
                                                         final_gen > 0.1 & final_sp > 0.1 ~ "Coexistence")) %>%
...
mus_c <- mu_c %>% mutate(coexistence = case_when(final_gen < 0.1 & final_sp < 0.1 ~ "Extinction",
                                                 final_gen > 0.1 & final_sp < 0.1 ~ "Generalist Only",
                                                 final_gen < 0.1 & final_sp > 0.1 ~ "Specialist Only",
                                                 final_gen > 0.1 & final_sp > 0.1 ~ "Coexistence"))%>%
...
alpha_gamma <- cycled_alphagamma %>% mutate(coexistence = case_when(final_gen < 0.1 & final_sp < 0.1 ~ "Extinction",
                                                                    final_gen > 0.1 & final_sp < 0.1 ~ "Generalist Only",
                                                                    final_gen < 0.1 & final_sp > 0.1 ~ "Specialist Only",
                                                                    final_gen > 0.1 & final_sp > 0.1 ~ "Coexistence")) %>%
...
alpha_c <- cycled_alphac %>% mutate(coexistence = case_when(final_gen < 0.1 & final_sp < 0.1 ~ "Extinction",
                                                            final_gen > 0.1 & final_sp < 0.1 ~ "Generalist Only",
                                                            final_gen < 0.1 & final_sp > 0.1 ~ "Specialist Only",
                                                            final_gen > 0.1 & final_sp > 0.1 ~ "Coexistence")) %>%
...
burst_size_resources <- cycled_gamma_coop %>% mutate(coexistence = case_when(final_gen < 0.1 & final_sp < 0.1 ~ "Extinction",
                                                                             final_gen > 0.1 & final_sp < 0.1 ~ "Generalist Only",
                                                                             final_gen < 0.1 & final_sp > 0.1 ~ "Specialist Only",
                                                                             final_gen > 0.1 & final_sp > 0.1 ~ "Coexistence")) %>%
...
consumption_resources <- cycled_c_coop %>% mutate(coexistence = case_when(final_gen < 0.1 & final_sp < 0.1 ~ "Extinction",
                                                                final_gen > 0.1 & final_sp < 0.1 ~ "Generalist Only",
                                                                final_gen < 0.1 & final_sp > 0.1 ~ "Specialist Only",
                                                                final_gen > 0.1 & final_sp > 0.1 ~ "Coexistence"))%>%
