#ATB
#coexistence graphs - competition
...
burst_size <- cycled_gamma_comp %>% mutate(coexistence = case_when(final_gen < 0.1 & final_sp < 0.1 ~ "Extinction",
...
consumption <- cycled_c_comp %>% mutate(coexistence = case_when(final_gen < 0.1 & final_sp < 0.1 ~ "Extinction",
                                                                final_gen > 0.1 & final_sp < 0.1 ~ "Generalist Only",
                                                                final_gen < 0.1 & final_sp > 0.1 ~ "Specialist Only",
                                                                final_gen > 0.1 & final_sp > 0.1 ~ "Coexistence")) %>%
...
beta <- cycled_beta %>% mutate(coexistence = case_when(final_gen < 0.1 & final_sp < 0.1 ~ "Extinction",
                                                       final_gen > 0.1 & final_sp < 0.1 ~ "Generalist Only",
                                                       final_gen < 0.1 & final_sp > 0.1 ~ "Specialist Only",
                                                       final_gen > 0.1 & final_sp > 0.1 ~ "Coexistence")) %>%
...
beta_gamma <- cycled_betagamma %>% mutate(coexistence = case_when(final_gen < 0.1 & final_sp < 0.1 ~ "Extinction",
                                                                  final_gen > 0.1 & final_sp < 0.1 ~ "Generalist Only",
                                                                  final_gen < 0.1 & final_sp > 0.1 ~ "Specialist Only",
                                                                  final_gen > 0.1 & final_sp > 0.1 ~ "Coexistence")) %>%
...
beta_c <- cycled_betac %>% mutate(coexistence = case_when(final_gen < 0.1 & final_sp < 0.1 ~ "Extinction",
                                                          final_gen > 0.1 & final_sp < 0.1 ~ "Generalist Only",
                                                          final_gen < 0.1 & final_sp > 0.1 ~ "Specialist Only",
                                                          final_gen > 0.1 & final_sp > 0.1 ~ "Coexistence")) %>%
...
burst_size_resources <- cycled_gamma_comp %>% mutate(coexistence = case_when(final_gen < 0.1 & final_sp < 0.1 ~ "Extinction",
                                                                             final_gen > 0.1 & final_sp < 0.1 ~ "Generalist Only",
                                                                             final_gen < 0.1 & final_sp > 0.1 ~ "Specialist Only",
                                                                             final_gen > 0.1 & final_sp > 0.1 ~ "Coexistence")) %>%
...
consumption_resources <- cycled_c_comp %>% mutate(coexistence = case_when(final_gen < 0.1 & final_sp < 0.1 ~ "Extinction",
                                                                          final_gen > 0.1 & final_sp < 0.1 ~ "Generalist Only",
                                                                          final_gen < 0.1 & final_sp > 0.1 ~ "Specialist Only",
                                                                          final_gen > 0.1 & final_sp > 0.1 ~ "Coexistence"))%>%
