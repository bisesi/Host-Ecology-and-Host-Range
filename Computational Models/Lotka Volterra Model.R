...
LVgeneralismcost <- function(model, parameters, modelname, startingdensity, timerange, maxcost, type){
  ...
  if (type == "gamma"){
    gamma_range <- seq(from = 0, to = parameters['gamma_sp'] * maxcost, by = parameters['gamma_sp'] / 20)
    ...
    for (i in 1:length(gamma_range)){
      ...
      total_fitness_gamma = rbind(total_fitness_gamma, cbind(repro_generalist = ((max(out$gen) - out$gen[1]) / out$gen[1]), repro_specialist = ((max(out$sp) - out$sp[1]) / out$sp[1]), cost = cost_type,
                                                             final_gen = tail(out$gen, n = 1), final_sp = tail(out$sp, n = 1), sp_burst = gamma_range[i]))
  ...
  if (type == "consumption"){
    c_range <- seq(from = 0, to = parameters['c_sp'] * maxcost, by = 0.00005)
    ...
    for (i in 1:length(c_range)){
      ...
      total_fitness_c = rbind(total_fitness_c, cbind(repro_generalist = ((max(out$gen) - out$gen[1]) / out$gen[1]), repro_specialist = ((max(out$sp) - out$sp[1]) / out$sp[1]), cost = cost_type,
                                                     final_gen = tail(out$gen, n = 1), final_sp = tail(out$sp, n = 1), sp_c = c_range[i]))
      c = rbind(c, out)
    }
...
LVgrowthrate <- function(model, parameters, modelname, startingdensity, timerange, maxcost, increment){
  ...
  if (modelname == "Cooperation"){
    e_growthrange <- seq(from = 0.3, to = parameters['rate_e'] * maxcost, by = increment)
  }
  else {
    e_growthrange <- seq(from = 0, to = parameters['rate_e'] * maxcost, by = increment)
  }
  ...
  for (i in 1:length(e_growthrange)){
    ...
    total_fitness_e = rbind(total_fitness_e, cbind(repro_generalist = ((max(out$gen) - out$gen[1]) / out$gen[1]), repro_specialist = ((max(out$sp) - out$sp[1]) / out$sp[1]), rate_cost = rate_cost,
                                                   final_gen = tail(out$gen, n = 1), final_sp = tail(out$sp, n = 1), e_mu = e_growthrange[i]))
